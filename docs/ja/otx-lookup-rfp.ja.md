...
実装と live 検証の過程で、起草時の前提が 3 点覆った。以下が現行の正典であり、
下の Discussion Log は起草時点の記録としてそのまま残す。

1. **ピボットに API キーは不要だった。** §2 は pulse 内 IoC 一覧をキー必須と
   書いているが、`GET /pulses/{id}` は匿名で通り、応答が `indicators` 配列を
   内包している。よってピボットはキー無しで機能する。キーが足すのは
   **正確な総数**（ページング endpoint の `count`）と検索とレート上限のみ。
2. **詳細応答は切り詰めない。** 埋め込み数とページング側 `count` は
   30/202/**4090** で一致した。上限に達すると切り詰めではなくタイムアウトする
   （約 335,000 IoC の pulse で発生）。したがって `IndicatorsExact=false` は
   「リストが短いかもしれない」ではなく「上流が総数を明示していない」を意味する。
3. **`exact_match` はドキュメントの bool ではなく string。** これに気付かず
   `bool` で宣言していたため、検索が全応答で decode に失敗していた。加えて
   検索の既定ソートは**古い順**で、トリアージには使えなかった（`sort=-modified`
   を明示するよう修正）。§7 に「キー必須の 2 endpoint は未実測」と書いた懸念が
   そのまま的中した形である。
